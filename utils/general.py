...
def apply_classifier(x, model, img, im0, det_classes_names, device):
    classifier_name_id = 7 # Emergency car index
    # Apply a second stage classifier to YOLO outputs
    # Example model = torchvision.models.__dict__['efficientnet_b0'](pretrained=True).to(device).eval()
    im0 = [im0] if isinstance(im0, np.ndarray) else im0
    for i, d in enumerate(x):  # per image
        if d is not None and len(d):
            if d[5] == classifier_name_id:
            #d = d.clone()
                d = np.expand_dims(d,axis=0)
                d = d.copy()
                # Reshape and pad cutouts
                #b = xyxy2xywh(d[:, :4])  # boxes
                #b[:, 2:] = np.expand_dims(b[:, 2:].max(1)[0], axis=1)  # rectangle to square
                #b[:, 2:] = b[:, 2:] * 1.3 + 30  # pad
                #d[:, :4] = xywh2xyxy(b).long()

                # Rescale boxes from img_size to im0 size
                scale_boxes(img.shape[2:], d[:, :4], im0[0].shape)

                # Classes
                #pred_cls1 = d[:, 5].long()
                ims = []
                for a in d:
                    cutout = im0[0][int(a[1]):int(a[3]), int(a[0]):int(a[2])]
                    im = cv2.resize(cutout, (224, 224))  # BGR

                    #im = im[:, :, ::-1].transpose(2, 0, 1)  # BGR to RGB, to 3x416x416
                    im = im.transpose(2, 0, 1)  # BGR to RGB, to 3x416x416
                    im = np.ascontiguousarray(im, dtype=np.float32)  # uint8 to float32
                    im /= 255  # 0 - 255 to 0.0 - 1.0
                    ims.append(im)
                
                pred_cls2 = model(torch.Tensor(ims).to(device)).argmax(1)  # classifier prediction
                #x[i] = x[i][pred_cls1 == pred_cls2]  # retain matching class detections
                x[i][5] = 9 + int(pred_cls2.cpu().numpy())
